...
/**
 * Create the given user view.
 *
 * @param {modle:user/userTable~UserTable} userView user view to create
 * @return {object} the result of {@link module:db/geoPackageConnection~GeoPackageConnection#run}
 */
TableCreator.prototype.createUserView = function (userView) {
  var connection = this.connection;

  var sqls = [
    'CREATE VIEW \'' + userView.view_name +
    '\' AS SELECT child.*, parent.geometry FROM \'' + userView.child_table_name +
    '\' child LEFT JOIN \'' + userView.parent_table_name +
    '\' parent ON child.parent_vetro_id = parent.vetro_id ' +
    'WHERE parent.geometry IS NOT NULL',

    'INSERT INTO gpkg_contents ' +
    '(table_name, data_type, identifier, description, last_change, ' +
    'min_x, min_y, max_x, max_y, srs_id) ' +
    'SELECT \'' + userView.view_name + '\', data_type, \'' + userView.view_name +
    '\', description, last_change, min_x, min_y, max_x, max_y, srs_id ' +
    'FROM gpkg_contents WHERE table_name = \'' + userView.parent_table_name + '\'',

    'INSERT INTO gpkg_geometry_columns ' +
    '(table_name, column_name, geometry_type_name, srs_id, z, m) ' +
    'SELECT \''+ userView.view_name +'\', column_name, geometry_type_name, srs_id, z, m ' +
    'FROM gpkg_geometry_columns WHERE table_name = \'' + userView.parent_table_name + '\''
  ];

  return sqls.reduce(function(sequence, sql) {
    return sequence.then(function() {
      try {
        return !!connection.run(sql);
      } catch (error) {
