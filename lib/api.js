...
GeoPackageAPI.createFeatureView = function(geopackage, viewName, childTableName, parentTableName) {
  var featureView = {
    view_name: viewName,
  ...
  return geopackage.createFeatureView(featureView);
}
...
GeoPackageAPI.createFeatureTableWithProperties = function(geopackage, tableName, properties) {
  ...
  var columns = [];
  var columnNumber = 0;
  columns.push(FeatureColumn.createPrimaryKeyColumnWithIndexAndName(columnNumber++, 'gpkg_id'));
...
GeoPackageAPI.createAttributeTableWithProperties = function(geopackage, tableName, properties) {
  var columns = [];
  var columnNumber = 0;
  columns.push(UserColumn.createPrimaryKeyColumnWithIndexAndName(columnNumber++, 'gpkg_id'));
...
GeoPackageAPI.addAttributeRow = function(geopackage, tableName, row) {
  ...
  attributeRow = attributeDao.newRowWithColumnTypes(attributeRow.columnTypes, row);
